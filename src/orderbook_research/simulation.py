...
def non_overlapping_signal_simulation(
    ...
    sampled["gross_return_bps"] = sampled["signal"] * sampled[f"future_return_bps_{horizon}"]
    sampled["estimated_cost_bps"] = sampled["signal"].abs() * (
        0.5 * sampled["spread_bps"] + 0.5 * sampled["future_spread_bps"] + additional_fee_bps
    )
    sampled["net_return_bps"] = sampled["gross_return_bps"] - sampled["estimated_cost_bps"]
    ...
    stats = {
        ...
        "gross_return_bps": float(sampled["gross_return_bps"].sum(skipna=True)),
        "net_return_bps": float(sampled["net_return_bps"].sum(skipna=True)),
        "mean_net_return_active_bps": float(
            sampled.loc[active, "net_return_bps"].mean() if active.any() else 0.0
        ),
        "max_drawdown_bps": float(sampled["drawdown_bps"].min(skipna=True)),
    }
